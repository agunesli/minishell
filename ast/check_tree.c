...
void	check_strong(t_syntax *syn, t_data *my_data)
{
	...
	{
		if (syn->id == OR)
			return(error_syntax("\'||\'", my_data)); //[2] bash: syntax error near unexpected token `|'
		else if (syn->id == AND)
			return(error_syntax("\'&&\'", my_data)); //[2] bash: syntax error near unexpected token `|'
		else if (syn->id == PIPE)
			return(error_syntax("\'|\'", my_data)); //[2] bash: syntax error near unexpected token `|'
...
void	check_medium(t_syntax *syn, t_data *my_data)
{
	(void)my_data;
	if (!syn->content)
		return(error_syntax("\'newline\'", my_data)); //[2] bash: syntax error near unexpected token `newline'
